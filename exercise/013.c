...
void test()
{
    ...
    for(i=0; i<start_id; ++i)
        printf("(%d,%d,%d)\n", i, StartList[i].id, StartList[i].last_id);
}
    ...
    if(can_escape(center, D+CENTER_DIA/2)){
        ...
        for(i=0; i<N; ++i){
            if(Corc[i].dist==Null && can_jump(center, Corc[i].Position, D+CENTER_DIA/2)){
                Corc[i].dist = 1;
                StartList[start_id].id = i;
                StartList[start_id].dist = Null;
                StartList[start_id].first_dist = cal_dist(center, Corc[i].Position);
                StartList[start_id].last_id = Null;
                start_id++;
                // printf("i=%d\n", i);
                // printf("add=%d\n", i);
            }
...
int search_best_way(PointType D)
{
    ...
    int i;
    for(i=0; i<start_id; ++i){
        if(StartList[i].last_id != Null){
            // printf("???\n");
            if(min_dist<0 || min_dist>StartList[i].dist || 
            (min_dist==StartList[i].dist && min_first_dist>StartList[i].first_dist)){
                min_id = i;
                min_dist = StartList[i].dist;
                min_first_dist = StartList[i].first_dist;
            }
        }
    }
    ...
    if(min_id!=Null){
        ...
        clean_node();
        BFS(min_id, D);
        return StartList[min_id].last_id;
    }else{
        return Null;
    }
}
...
void clean_node()
{
    ...
    for(i=0; i<start_id; ++i){
        Corc[StartList[i].id].dist = 1;
    }
    while(!IsEmpty(Q)) DeleteQ(Q);
}
...
void BFS(int s_id, PointType D)
{
    int now_id;
    int i;
    AddQ(Q, StartList[s_id].id);
    while(!IsEmpty(Q)){
        now_id = DeleteQ(Q);
        // printf("now_id=%d\n", now_id);
        if(can_escape(Corc[now_id].Position, D)){
            // printf("s_id%d, now_id%d\n", s_id, now_id);
            StartList[s_id].last_id = now_id;
            StartList[s_id].dist = Corc[now_id].dist + 1;
            break;
        }else{
            for(i=0; i<N; ++i){
                if(Corc[i].dist==Null && can_jump(Corc[now_id].Position, Corc[i].Position, D)){
